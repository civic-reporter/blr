...
export async function displayTrafficSuccessInfo() {
    ...
    if (successInfoDiv && (wardNo || corpName)) {
        ...
        if (trafficPS) {
            html += `<div style="margin-bottom: 0.5rem; color: var(--x-text-primary);"><strong>🚔 Traffic PS:</strong> ${trafficPS}</div>`;
        }
        ...
        if (psName) {
            ...
            if (contactInfo && (contactInfo.mobile || contactInfo.landline)) {
                html += `<div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--x-border); color: var(--x-text-primary);"><strong>For urgent issues, contact ${psName}:</strong></div>`;
                html += '<div style="display: flex; gap: 1rem; flex-wrap: wrap; margin-top: 0.5rem;">';
                if (contactInfo.mobile) {
                    html += `<a href="tel:${contactInfo.mobile}" style="display: inline-flex; align-items: center; padding: 0.5rem 1rem; background: var(--x-accent); color: white; text-decoration: none; border-radius: 8px; font-weight: 500; box-shadow: var(--shadow);">📱 ${contactInfo.mobile}</a>`;
                }
                if (contactInfo.landline) {
                    html += `<a href="tel:${contactInfo.landline}" style="display: inline-flex; align-items: center; padding: 0.5rem 1rem; background: var(--x-accent); color: white; text-decoration: none; border-radius: 8px; font-weight: 500; box-shadow: var(--shadow);">☎️ ${contactInfo.landline}</a>`;
                }
                html += '</div>';
            }
        }
